...
select ok(
  exists (select 1 from information_schema.columns where table_schema='public' and table_name='subscriptions' and column_name='provider'),
  'subscription ledger has provider identity'
);
select ok(
  exists (select 1 from information_schema.columns where table_schema='public' and table_name='subscriptions' and column_name='entitlement'),
  'subscription ledger has canonical entitlement identity'
);
select ok(
  exists (select 1 from pg_indexes where schemaname='public' and tablename='subscriptions' and indexname='subscriptions_provider_identifier_idx'),
  'provider subscription identity is uniquely indexed'
);
...
select throws_ok(
  $$select public.sync_revenuecat_entitlements_v1('client-forge','INITIAL_PURCHASE','1a000000-0000-0000-0000-000000000001',now(),array['creator']::text[],null)$$,
  '42501',
  null,
  'authenticated client cannot forge RevenueCat state'
);
select throws_ok(
  $$update public.subscriptions set status='active' where user_id='1a000000-0000-0000-0000-000000000001'$$,
  '42501',
  null,
...
select is(
  public.sync_revenuecat_entitlements_v1(
    'rc-proof-plus','INITIAL_PURCHASE','1a000000-0000-0000-0000-000000000001',
    '2026-08-25T06:00:00Z'::timestamptz,array['proof_plus']::text[],'https://example.test/manage-a'
  ),
  true,
  'service sync accepts validated Proof+ snapshot'
);
reset role;
...
select is(
  public.sync_revenuecat_entitlements_v1(
    'rc-creator','RENEWAL','1a000000-0000-0000-0000-000000000001',
    '2026-08-25T07:00:00Z'::timestamptz,array['creator']::text[],'https://example.test/manage-a'
  ),
  true,
  'newer Creator snapshot is accepted'
);
reset role;
...
select is(
  public.sync_revenuecat_entitlements_v1(
    'rc-delayed','EXPIRATION','1a000000-0000-0000-0000-000000000001',
    '2026-08-25T06:30:00Z'::timestamptz,array[]::text[],null
  ),
  true,
  'delayed unique event is consumed without replaying state'
);
reset role;
...
set local role service_role;
select throws_ok(
  $$select public.sync_revenuecat_entitlements_v1('rc-black','INITIAL_PURCHASE','1a000000-0000-0000-0000-000000000001',now(),array['black']::text[],null)$$,
  'P0001',
...
select is(
  public.sync_revenuecat_entitlements_v1(
    'rc-black-empty','EXPIRATION','1a000000-0000-0000-0000-000000000003',
    '2026-08-25T08:00:00Z'::timestamptz,array[]::text[],null
  ),
  true,
  'Black account may receive ordinary RevenueCat lifecycle events'
);
reset role;
...
set local role service_role;
select ok(
  public.sync_stripe_subscription_event(
    'evt_legacy_pro','customer.subscription.created','sub_legacy_pro','cus_legacy_pro',
    '1a000000-0000-0000-0000-000000000002','active','pro'
  ),
  'legacy Stripe Proof+ remains a valid entitlement source'
);
select ok(
  public.sync_revenuecat_entitlements_v1(
    'rc-b-creator','INITIAL_PURCHASE','1a000000-0000-0000-0000-000000000002',
    '2026-08-25T09:00:00Z'::timestamptz,array['creator']::text[],null
  ),
  'RevenueCat Creator can coexist with a legacy Stripe Proof+ subscription'
);
reset role;
...
set local role service_role;
select public.sync_revenuecat_entitlements_v1(
  'rc-b-expired','EXPIRATION','1a000000-0000-0000-0000-000000000002',
  '2026-08-25T10:00:00Z'::timestamptz,array[]::text[],null
);
reset role;
