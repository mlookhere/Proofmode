...
set local role anon;
select ok(public.get_public_post_share_v1('47000000-0000-0000-0000-000000000001') is not null,'public post has a share snapshot');
...
select is(public.get_public_post_share_v1('47000000-0000-0000-0000-000000000002'),null::jsonb,'unpublished post is not shareable');
...
select is(public.get_public_receipt_share_v1('57000000-0000-0000-0000-000000000002'),null::jsonb,'private receipt is not shareable');
select is(public.get_public_receipt_share_v1('57000000-0000-0000-0000-000000000001')->>'id','57000000-0000-0000-0000-000000000001','canonical Receipt ID is the proof ID');
select is(public.resolve_invite_share_v1('shareinvite001')->>'challenge_slug','private-share','valid invite capability resolves its target');
select is(public.resolve_invite_share_v1('missinginvite'),null::jsonb,'unknown invite does not resolve');
reset role;

select is((select count(*)::int from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname=any(array['get_public_post_share_v1','get_public_receipt_share_v1','resolve_invite_share_v1']) and not p.prosecdef),3,'public share RPCs are invoker wrappers');
...
select is(has_table('public','receipts'),false,'sharing does not create a duplicate Receipt table');
...
select is(public.get_public_post_share_v1('47000000-0000-0000-0000-000000000001'),null::jsonb,'blocked viewer cannot load canonical post share');
