...
import { recordInviteClaimed, shareCanonical } from "@/sharing";
...
export default function ChallengeScreen() {
  ...
  const { slug, action, ref, src } = useLocalSearchParams<{ slug: string; action?: PendingAction; ref?: string; src?: string }>();
  ...
  const inviteCode = ref?.trim() || null;
  const source = src?.trim() || "drop_link";

  const authReturnTo = useCallback((nextAction?: PendingAction) => {
    if (!slug) return "/(tabs)/explore";
    const params = new URLSearchParams();
    if (nextAction) params.set("action", nextAction);
    if (inviteCode) params.set("ref", inviteCode);
    if (source) params.set("src", source);
    const query = params.toString();
    return `/challenge/${slug}${query ? `?${query}` : ""}`;
  }, [inviteCode, slug, source]);

  const load = useCallback(async () => {
    if (!isSupabaseConfigured || !slug) { setIsLoading(false); return; }
    setIsLoading(true);
    setError(null);
    try {
  ...
  const runAction = useCallback(async (nextAction: PendingAction) => {
    if (!challenge) return;
    if (!session) {
      router.push({ pathname: "/auth", params: { returnTo: authReturnTo(nextAction) } });
      return;
    }
    ...
    try {
      if (nextAction === "join") {
        const challengeId = await joinChallenge(challenge.slug, inviteCode);
        if (inviteCode) await recordInviteClaimed(inviteCode, challengeId);
      } else {
  ...
  }, [authReturnTo, challenge, inviteCode, router, session, viewerState.watched]);

  useEffect(() => {
    if (!session || !challenge || (action !== "join" && action !== "watch")) return;
    const key = `${session.user.id}:${challenge.id}:${action}:${inviteCode || ""}`;
    if (handledAction.current === key) return;
    handledAction.current = key;
    void runAction(action);
  }, [action, challenge, inviteCode, runAction, session]);

  async function openJourney() {
    if (!challenge || isMutating) return;
    if (!session) {
      router.push({ pathname: "/auth", params: { returnTo: authReturnTo() } });
      return;
    }
  ...
  async function shareDrop() {
    ...
    try {
      await shareCanonical({
        ...
        text: challenge.tagline || challenge.rule,
        path: `/c/${challenge.slug}`,
        source: "drop_share",
      });
    } catch { setError("Could not open share sheet."); }
  }

  if (!isSupabaseConfigured) return <Screen contentStyle={styles.centered}><Eyebrow>BACKEND NOT CONFIGURED</Eyebrow><Text style={styles.title}>DROP ACTIONS ARE READY.</Text><Text style={styles.copy}>Configure Supabase to load live Drops and persist Join or Watch state.</Text></Screen>;
  if (isLoading) return <Screen contentStyle={styles.centered}><ActivityIndicator color={colors.hot} /></Screen>;
  if (!challenge) return <Screen contentStyle={styles.centered}><Eyebrow>DROP NOT FOUND</Eyebrow><Text style={styles.title}>THIS ONE ISN’T PUBLIC.</Text><PrimaryButton onPress={() => router.replace("/(tabs)/explore")}>BACK TO EXPLORE</PrimaryButton></Screen>;
  ...
  return <>
    <Screen>
      ...
      <Surface style={styles.details}>
        ...
        <View style={styles.metaRow}><Text style={styles.meta}>{challenge.durationDays} DAYS</Text><Text style={styles.meta}>{challenge.category.toUpperCase()}</Text></View>
      </Surface>

      {journey ? <Surface style={styles.journeySummary}><Text style={styles.detailLabel}>YOUR JOURNEY</Text><Text style={styles.journeyValue}>ATTEMPT #{journey.attempt_no} · {journey.verified_count} VERIFIED</Text><Text style={styles.copySmall}>Current streak {journey.current_streak} · Best {journey.best_streak}</Text></Surface> : null}
      {error ? <Text style={styles.error}>{error}</Text> : null}

      <View style={styles.actions}>
        <PrimaryButton onPress={isMutating ? undefined : () => void openJourney()}>{isMutating ? "WORKING…" : journey ? "CONTINUE JOURNEY" : viewerState.joined ? "START JOURNEY" : "JOIN + START"}</PrimaryButton>
        <PrimaryButton onPress={isMutating || viewerState.joined ? undefined : () => void runAction("join")} style={styles.secondaryButton} textStyle={styles.secondaryButtonText}>{viewerState.joined ? "JOINED" : "JOIN DROP"}</PrimaryButton>
        <PrimaryButton onPress={isMutating ? undefined : () => void runAction("watch")} style={styles.secondaryButton} textStyle={styles.secondaryButtonText}>{viewerState.watched ? "UNWATCH" : "WATCH"}</PrimaryButton>
        <PrimaryButton onPress={() => void shareDrop()} style={styles.secondaryButton} textStyle={styles.secondaryButtonText}>SHARE DROP</PrimaryButton>
        <PrimaryButton onPress={openReport} style={styles.reportButton} textStyle={styles.secondaryButtonText}>REPORT DROP</PrimaryButton>
      </View>
