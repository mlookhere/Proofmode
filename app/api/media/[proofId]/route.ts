...
export async function GET(request: Request, { params }: { params: Promise<{ proofId: string }> }) {
  ...
  const { data: proof, error } = await userClient.from("proofs").select("media_url").eq("id", proofId).single();
  if (error || !proof?.media_url) return NextResponse.json({ error: "Receipt not found" }, { status: 404 });
