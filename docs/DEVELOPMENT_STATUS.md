...
### Active development — Journey, Proof Ledger, and Passport MVP
- Issue #23 is active on `work/23-journey-proof-passport`; draft PR #24 targets `dev`.
- Migration `015_journey_proof_passport.sql` is forward-only after staging migration `014` and remains unapplied to staging until the exact branch head passes the Database CI gate.
- The branch reuses the existing `journeys`, `proofs`, `verifications`, `posts`, challenge membership, and media lifecycle rather than creating a parallel reputation model.
- The database contract adds one-active-attempt enforcement, retry-safe Reset tokens, Journey follows, publication-driven proof linking for eligible Proof/Comeback/PR posts, block-aware Journey reads, challenge-member proof verification, streak metrics, Journey snapshots, and expanded Passport metrics/history.
- Fail, Almost, and Reset remain story content and do not silently become proof receipts; social `Proven` remains separate from proof verification.
- The v3 upload-intent path now assigns the caller's Journey through authenticated RPCs while media/post lifecycle state remains server-owned; pending Reset uploads retain their idempotency token across retry.
- Legacy web proof creation and verification are being routed through the same credibility RPC boundary instead of direct table mutation.
- Mobile branch work includes live Journey navigation/timeline, Follow Journey, Join/Start same Drop, Run It Back, member Verify/Reject, feed Journey doorway, and expanded You/Passport metrics/history.
- Transactional Journey/Passport pgTAP and Foundation validation are being expanded on the branch. Do not represent this slice as CI-verified, staging-verified, integrated, release-ready, or shipped until those gates pass.
...
1. **Finish Journey/proof/Passport integration** — branch CI, staging migration/advisors/rollback smoke, final control pass, then merge Issue #23 into `dev`.
